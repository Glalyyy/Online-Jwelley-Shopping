    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Index(string password, string username)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var data_cus = db.UserRegMst.Where(s => s.Username == username && s.password.Equals(f_password));
                var data_ad = db.AdminLoginMst.Where(s => s.userName == username && s.Password.Equals(f_password));
        ...
        public ActionResult Register(FormCollection form, UserRegMst user, string username)
        {
            if (ModelState.IsValid)
            {
                var check = db.UserRegMst.FirstOrDefault(s => s.Username == username);
                if (check == null)
                {
        ...
        public ActionResult Information()
        {
            ...
            UserRegMst users = db.UserRegMst.Where(s => s.Username == username).FirstOrDefault();
            return View(users);
        }
        [HttpPost]
        public ActionResult Information(UserRegMst users, HttpPostedFileBase imageFiles)
        {
            ...
            var username = Session["Username"].ToString();
            var testuser = db.UserRegMst.FirstOrDefault(s => s.Username == username);
            if (testuser != null)
            {
                UserRegMst user = db.UserRegMst.Where(row => row.Username == username).FirstOrDefault();
        ...
        public ActionResult Change(string Password, UserRegMst userReg, string New)
        {
            ...
            var data = db.UserRegMst.Where(s => s.Username == username && s.password == cpass);
            if (data != null)
            {

                var cus = db.UserRegMst.Where(s => s.Username == username && s.password == newpass).FirstOrDefault();
                if (cus == null)
                {
                    var users = db.UserRegMst.FirstOrDefault(s => s.Username == username);
